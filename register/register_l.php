...
class register_l{
    public function newRegister($form,$facebook){
        $user = $facebook->api('/me?fields=id,name,birthday,gender,hometown','GET');
        ...
        if($tmp){
            ...
            if($db->insert("m_user",array("facebook_id"=>$user["id"],"birthday"=>$elms[2]."-".$elms[0]."-".$elms[1],"sex"=>$user["gender"],"hometown"=>$user["hometown"]["name"]))){
                return "新規登録しました";
            }else{
                return "登録に失敗しました";
            }
